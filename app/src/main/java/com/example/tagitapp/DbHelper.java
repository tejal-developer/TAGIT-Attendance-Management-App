...
public class DbHelper extends SQLiteOpenHelper {
    ...
    //      Student Table
    static final String STUDENT_TABLE_NAME = "STUDENT_TABLE";
...
//      STATUS TABLE
    ...
    public void onCreate(SQLiteDatabase db) {
        try{     db.execSQL(CREATE_CLASS_TABLE);
            db.execSQL(CREATE_STUDENT_TABLE);
            db.execSQL(CREATE_STATUS_TABLE);}catch (SQLException e){
            e.printStackTrace();
        }
    }
    ...
    public boolean checkStudentExists(int roll) {
        SQLiteDatabase database = this.getReadableDatabase();
        Cursor cursor = null;

        try {
            String query = "SELECT * FROM " + STUDENT_TABLE_NAME + " WHERE " + STUDENT_ROLL_KEY + " = ?";
            cursor = database.rawQuery(query, new String[]{String.valueOf(roll)});
            return cursor.moveToFirst(); // Returns true if a student with the given roll exists
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    public boolean isDataPresent() {
        ...
        try {
            String query = "SELECT EXISTS (SELECT 1 FROM " + CLASS_TABLE_NAME + " LIMIT 1)";
            cursor = database.rawQuery(query, null);
            if (cursor.moveToFirst()) {
                int exists = cursor.getInt(0);
                return exists == 1; // Returns true if there is at least one row in the CLASS_TABLE
            }
            return false;
        } finally {
            if (cursor != null) {
